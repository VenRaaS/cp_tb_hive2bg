...
export TABLE_NM="Goods_"${EX_DATE}
...
hive -e "create table ${HIVE_TMP_DB}.${TABLE_NM}
    ...
    AS
    select gid,pgid,goods_name,goods_keyword,goods_brand,goods_describe,goods_spec,goods_img_url,availability,currency,sale_price,provider,barcode_ean13,barcode_upc,first_rts_date,unix_timestamp(update_time) as update_time,unix_timestamp(sys_update_time) as sys_update_time from momo_unima.unima_goods where to_date(sys_update_time) = '${EX_DATE10}'"
...
bq query --project_id ${GCP_PROJECT_ID} --nouse_legacy_sql --allow_large_results --destination_table=${BQ_DB}.${TABLE_NM} "select gid,pgid,goods_name,goods_keyword,goods_brand,goods_describe,goods_spec,goods_img_url,availability,currency,sale_price,provider,barcode_ean13,barcode_upc,first_rts_date,DATETIME(TIMESTAMP_SECONDS(update_time), '+08:00') as update_time,DATETIME(TIMESTAMP_SECONDS(sys_update_time), '+08:00') as sys_update_time FROM ${BQ_TMP_DB}.${TABLE_NM}"
