...
export TABLE_NM="GoodsCateCode_"${EX_DATE}
...
hive -e "create table ${HIVE_TMP_DB}.${TABLE_NM}
    ...
    AS
    select gid,category_code,le,sort,func_type,insert_date,desplay_start_date,desplay_end_date,unix_timestamp(update_time) as update_time,unix_timestamp(sys_update_time) as sys_update_time from momo_unima.unima_goods_cate_code where to_date(sys_update_time) = '${EX_DATE10}'"
...
bq query --project_id ${GCP_PROJECT_ID} --nouse_legacy_sql --allow_large_results --destination_table=${BQ_DB}.${TABLE_NM} "select gid,category_code,le,sort,func_type,insert_date,desplay_start_date,desplay_end_date,DATETIME(TIMESTAMP_SECONDS(update_time), '+08:00') as update_time,DATETIME(TIMESTAMP_SECONDS(sys_update_time), '+08:00') as sys_update_time FROM ${BQ_TMP_DB}.${TABLE_NM}"
