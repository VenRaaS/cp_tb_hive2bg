...
export TABLE_NM="GoodsCategoryFlatten_"${EX_DATE}
...
hive -e "create table ${HIVE_TMP_DB}.${TABLE_NM}
    ...
    AS
    select gid,category_code,p_category_code,le,unix_timestamp(sys_update_time) as sys_update_time 
    from momo_unima.goods_category_flatten
...
bq query --project_id ${GCP_PROJECT_ID} --nouse_legacy_sql --allow_large_results --destination_table=${BQ_DB}.${TABLE_NM} "select gid,category_code,p_category_code,le,DATETIME(TIMESTAMP_SECONDS(sys_update_time), '+08:00') as sys_update_time FROM ${BQ_TMP_DB}.${TABLE_NM}"
